...
typedef struct _referencedString
{
...
typedef struct _funcDefinition
{
...
// referenced functions inside the function definitions of the gsc
typedef struct _referencedFunction
{
...
typedef struct _COD9_GSC
{
	...
	DWORD codeSectionStart;		// 0x14 - A relative pointer to where the code section starts, may start with padding before a function definiton
	DWORD referencedStrings;	// 0x18 - A relative pointer to an array of referencedString structs, amount = numOfRefStrings
	DWORD funcDefinitions;		// 0x1C - A relative pointer to an array of funcDefinition structs, amount = numOfFuncDefinitions
	DWORD referencedFunctions;	// 0x20 - A relative pointer to an array of referencedFunction structs, amount = numOfRefFunctions
	DWORD relocations;			// 0x24 - A relative pointer to an array of relocation structs, amount = numOfRelocations
	DWORD size;					// 0x28 - Size of the gsc file
	DWORD codeSectionSize;		// 0x2C - Size of the code section
	WORD name;					// 0x30 - A relative pointer to the name of this gsc (with full path and extension, null terminated)
	WORD numOfRefStrings;		// 0x32 - Number of strings referenced inside the function definitions
